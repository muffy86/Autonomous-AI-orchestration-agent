...
export const documentAnalyzer = tool({
  description: 'Analyze documents for structure, content, quality, and provide insights and suggestions',
  parameters: z.object({
    content: z.string().min(1, 'Document content is required'),
    analysisType: z.enum(['full', 'summary', 'structure', 'quality', 'insights']).default('full'),
    language: z.string().optional().describe('Document language (auto-detected if not provided)'),
    documentType: z.string().optional().describe('Type of document (auto-detected if not provided)'),
  }),
  execute: async ({ content, analysisType, language, documentType }) => {
    try {
      ...
      const analysis = await analyzeDocument(content, analysisType, detectedLanguage, detectedType);
...
function detectDocumentType(content: string): string {
  const typePatterns = {
    'academic': [/abstract/i, /introduction/i, /methodology/i, /conclusion/i, /references/i, /bibliography/i],
    'technical': [/api/i, /function/i, /parameter/i, /configuration/i, /installation/i, /usage/i],
    'business': [/executive summary/i, /market/i, /revenue/i, /strategy/i, /proposal/i, /budget/i],
    'legal': [/whereas/i, /hereby/i, /pursuant/i, /agreement/i, /contract/i, /terms/i],
...
function analyzeStructure(content: string, paragraphs: string[]): DocumentAnalysisResult['structure'] {
  ...
  if (headings.length > 0) {
    for (let i = 0; i < headings.length; i++) {
      ...
      const sectionWords = sectionContent.split(/\s+/).filter(w => w.length > 0);
...
function extractKeyPoints(sentences: string[]): string[] {
  ...
  sentences.forEach(sentence => {
    ...
    const isKeyPoint = keyIndicators.some(pattern => pattern.test(trimmed)) ||
...
function extractEntities(content: string): Array<{ type: string; value: string; confidence: number }> {
  const entities: Array<{ type: string; value: string; confidence: number }> = [];

  // Extract dates
  const dateRegex = /\b\d{1,2}\/\d{1,2}\/\d{4}\b|\b\d{4}-\d{2}-\d{2}\b|\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}\b/g;
...
function analyzeSentiment(content: string): DocumentAnalysisResult['content']['sentiment'] {
  const positiveWords = [
    ...
    'success', 'achieve', 'accomplish', 'win', 'victory', 'triumph', 'benefit', 'advantage',
    'improve', 'enhance', 'optimize', 'effective', 'efficient', 'productive', 'valuable'
  ];

  const negativeWords = [
    'bad', 'terrible', 'awful', 'horrible', 'disgusting', 'disappointing', 'frustrating',
    'annoying', 'irritating', 'infuriating', 'outrageous', 'unacceptable', 'inadequate',
    'fail', 'failure', 'lose', 'loss', 'defeat', 'problem', 'issue', 'difficulty',
    'challenge', 'obstacle', 'barrier', 'limitation', 'weakness', 'flaw', 'defect'
  ];
...
function analyzeQuality(content: string, sentences: string[], words: string[]): DocumentAnalysisResult['quality'] {
  const issues: DocumentAnalysisResult['quality']['issues'] = [];
  const suggestions: string[] = [];

  // Calculate readability score (simplified Flesch Reading Ease)
  const avgSentenceLength = words.length / sentences.length;
  const avgSyllables = words.reduce((sum, word) => sum + countSyllables(word), 0) / words.length;
  ...
  if (repetitiveWords.length > 0) {
    suggestions.push(`Consider varying word choice. Frequently repeated words: ${repetitiveWords.slice(0, 3).join(', ')}`);
  }

  // General suggestions based on readability
  if (readabilityScore < 50) {
    suggestions.push('Consider simplifying sentence structure and using shorter sentences');
...
function generateInsights(
  ...
  const informalWords = ['really', 'pretty', 'quite', 'very', 'just', 'actually'];
  const technicalWords = ['system', 'process', 'method', 'analysis', 'implementation'];

  const lowerContent = content.toLowerCase();
  const formalCount = formalWords.filter(word => lowerContent.includes(word)).length;
  const informalCount = informalWords.filter(word => lowerContent.includes(word)).length;
