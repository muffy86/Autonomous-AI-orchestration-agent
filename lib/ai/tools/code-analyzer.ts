...
export const codeAnalyzer = tool({
  description:
    'Analyze code for quality, security, performance, and provide optimization suggestions',
  parameters: z.object({
...
async function analyzeCode(
  ...
  if (includeOptimizations) {
    analysis.quality.suggestions.push(
      ...generateOptimizations(code, language, analysis),
    );
  }
...
function analyzeComplexity(
  ...
  const functionCount = (code.match(/function\s+\w+|def\s+\w+|fn\s+\w+/g) || [])
    .length;
...
function analyzeQuality(
  ...
  lines.forEach((line, index) => {
    ...
    if (
      magicNumberRegex.test(trimmedLine) &&
  ...
  const qualityScore = Math.max(
    0,
    Math.min(10, 10 - (issueWeight / totalLines) * 10),
  );
...
function analyzeSecurity(
  ...
  const securityScore = Math.max(
    0,
    10 - (criticalCount * 4 + highCount * 2 + mediumCount * 1),
  );
...
function analyzePerformance(
  ...
  lines: string[],
  language: string,
): CodeAnalysisResult['performance'] {
  ...
  if (code.includes('for') && code.includes('for')) {
    ...
    if (nestedLoops > 0) {
      bottlenecks.push(
        `${nestedLoops} nested loop(s) detected - O(n²) complexity`,
      );
      optimizations.push(
  ...
  if (language === 'javascript' || language === 'typescript') {
    const domQueryMatches = code.match(/document\.getElementById/g);
    if (
      code.includes('document.getElementById') &&
      domQueryMatches &&
      domQueryMatches.length > 3
    ) {
    ...
    if (code.includes('JSON.parse') && code.includes('JSON.stringify')) {
      optimizations.push(
        'Consider using more efficient serialization methods for large objects',
      );
    }
  }
  ...
  if (language === 'python') {
    ...
    if (code.includes('range(len(')) {
      optimizations.push(
        'Use enumerate() instead of range(len()) for better readability and performance',
      );
    }
  }
...
function analyzeDocumentation(
  ...
  lines: string[],
  language: string,
): CodeAnalysisResult['documentation'] {
  ...
  const functionRegex =
    functionRegexes[language as keyof typeof functionRegexes];
  ...
  if (functionRegex && docRegex) {
    ...
    const coverage =
      functions.length > 0
        ? (docComments.length / functions.length) * 100
        : 100;
...
function generateOptimizations(
  ...
  if (analysis.security.vulnerabilities.length > 0) {
    optimizations.push(
      'Fix security vulnerabilities before deploying to production',
    );
  }
  ...
  if (analysis.documentation.coverage < 50) {
    optimizations.push(
      'Improve code documentation for better team collaboration',
    );
  }
