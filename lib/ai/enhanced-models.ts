...
export const enhancedChatModels: EnhancedChatModel[] = [
  {
    ...
    name: 'Grok 2 Vision',
    description:
      'Advanced multimodal model with vision capabilities and real-time information',
    provider: 'xAI',
    version: '1212',
    capabilities: {
      textGeneration: true,
  ...
  },
  {
    ...
    description:
      'Compact reasoning model optimized for logical thinking and problem-solving',
    provider: 'xAI',
    version: 'beta',
    capabilities: {
      textGeneration: true,
...
export class ModelManager {
  ...
  private usage: Map<
    string,
    { requests: number; tokens: number; lastReset: Date }
  >;
  ...
  getAvailableModels(): EnhancedChatModel[] {
    return this.getAllModels().filter(
      (model) => model.isAvailable && !model.deprecated,
    );
  }

  getModelsByCategory(
    category: keyof typeof modelCategories,
  ): EnhancedChatModel[] {
      ...
      case 'coding':
        return (
          models
            ...
            .sort(
              (a, b) =>
                b.capabilities.contextLength - a.capabilities.contextLength,
            )[0] || null
        );
      ...
      case 'reasoning':
        return (
          models
            ...
            .sort((a, b) => {
              if (a.category === 'reasoning' && b.category !== 'reasoning')
                return -1;
              if (b.category === 'reasoning' && a.category !== 'reasoning')
                return 1;
              return (
                b.capabilities.contextLength - a.capabilities.contextLength
              );
  ...
  checkRateLimit(
    ...
    const minutesSinceReset =
      (now.getTime() - usage.lastReset.getTime()) / (1000 * 60);
    ...
    const requestsRemaining = Math.max(
      0,
      model.limits.requestsPerDay - usage.requests,
    );
    const tokensRemaining = Math.max(
      0,
      model.limits.tokensPerDay - usage.tokens,
    );
  ...
  getUsageStats(
    modelId: string,
  ): { requests: number; tokens: number; lastReset: Date } | null {
...
export const modelSelectionSchema = z.object({
  ...
  task: z
    .enum(['general', 'coding', 'reasoning', 'vision', 'creative', 'analysis'])
    .optional(),
...
export const modelComparisonSchema = z.object({
  ...
  criteria: z
    .array(z.enum(['capabilities', 'pricing', 'performance', 'limits']))
    .optional(),
});
