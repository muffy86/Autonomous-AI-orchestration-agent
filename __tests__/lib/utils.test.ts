...
describe('utils', () => {
  ...
  describe('fetchWithErrorHandlers', () => {
    ...
    it('should throw ChatSDKError when response is not ok', async () => {
      ...
      await expect(fetchWithErrorHandlers('/api/test')).rejects.toThrow(ChatSDKError)
    })
    ...
    it('should rethrow other errors', async () => {
      ...
      await expect(fetchWithErrorHandlers('/api/test')).rejects.toThrow(customError)
    })
  })
  ...
  describe('getMostRecentUserMessage', () => {
    it('should return the most recent user message', () => {
      ...
      expect(result).toEqual({ id: '3', role: 'user', content: 'Second message' })
    })
