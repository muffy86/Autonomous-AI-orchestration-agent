...
function analyzeBundle() {
  ...
  try {
    ...
    if (!fs.existsSync(nextDir)) {
      console.error(
        '❌ .next directory not found. Please run "pnpm build" first.',
      );
      process.exit(1);
    }
...
function analyzeStaticFiles(staticDir) {
  ...
  chunks.slice(0, 10).forEach((chunk, index) => {
    ...
    if (
      chunk.path.includes('framework') ||
      chunk.path.includes('main') ||
      chunk.path.includes('webpack')
    ) {
  ...
  if (firstLoadJS > BUNDLE_SIZE_LIMITS.firstLoadJS) {
    console.log(
      `⚠️  First Load JS exceeds recommended limit (${formatBytes(BUNDLE_SIZE_LIMITS.firstLoadJS)})`,
    );
  } else {
  ...
  if (totalSize > BUNDLE_SIZE_LIMITS.totalError) {
    console.log(
      `❌ Total bundle size is very large (>${formatBytes(BUNDLE_SIZE_LIMITS.totalError)})`,
    );
  } else if (totalSize > BUNDLE_SIZE_LIMITS.totalWarning) {
    console.log(
      `⚠️  Total bundle size is getting large (>${formatBytes(BUNDLE_SIZE_LIMITS.totalWarning)})`,
    );
  } else {
