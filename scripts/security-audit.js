...
class SecurityAuditor {
  ...
  checkHardcodedSecrets(content, filePath) {
    const secretPatterns = [
      { pattern: /(?:password|pwd|pass)\s*[:=]\s*['"][^'"]{8,}['"]/, name: 'Hardcoded password' },
      { pattern: /(?:api[_-]?key|apikey)\s*[:=]\s*['"][^'"]{16,}['"]/, name: 'Hardcoded API key' },
      { pattern: /(?:secret|token)\s*[:=]\s*['"][^'"]{16,}['"]/, name: 'Hardcoded secret/token' },
      { pattern: /(?:private[_-]?key|privatekey)\s*[:=]\s*['"][^'"]{32,}['"]/, name: 'Hardcoded private key' },
      { pattern: /(?:auth[_-]?secret|authsecret)\s*[:=]\s*['"][^'"]{16,}['"]/, name: 'Hardcoded auth secret' },
    ];
  ...
  checkInsecurePatterns(content, filePath) {
    const insecurePatterns = [
      ...
      { pattern: /console\.log\(.*(?:password|secret|token|key).*\)/i, severity: 'medium', description: 'Sensitive data in console logs' },
      { pattern: /http:\/\/(?!localhost|127\.0\.0\.1)/, severity: 'low', description: 'Insecure HTTP connection' },
      { pattern: /setTimeout\s*\(\s*['"][^'"]*['"]/, severity: 'medium', description: 'Code execution via setTimeout string' },
      { pattern: /setInterval\s*\(\s*['"][^'"]*['"]/, severity: 'medium', description: 'Code execution via setInterval string' },
    ];
  ...
  checkInputValidation(content, filePath) {
    ...
    if (filePath.includes('/api/') && filePath.endsWith('.ts')) {
      const hasValidation = /z\./.test(content) || /validate/.test(content) || /schema/.test(content);
      const hasUserInput = /request\.json\(\)/.test(content) || /formData\.get/.test(content);
